...
# Import our Listening Room module
from listening_room.listening_room import run_listening_room
...
def main():
    ...
    # Initialize ElevenLabs specific variables
    if "elevenlabs_voice_models" not in st.session_state:
        st.session_state.elevenlabs_voice_models = {}

    if "elevenlabs_voice_name" not in st.session_state:
        st.session_state.elevenlabs_voice_name = ""

    # Initialize OpenAI specific variables
    if "openai_voice" not in st.session_state:
        st.session_state.openai_voice = "alloy"

    # Initialize flags for built-in API keys
    ...
    tab = st.radio(
        ...
        ["✨ Voice Generation", "🌍 Voice Dubbing", "🎧 Listening Room", "ℹ️ About"],
        horizontal=True,
...
Julie (optimistic): Though that new camera system might be a game-changer for mobile photography."""
    ...
    # About tab
    # Listening Room Tab
    elif tab == "🎧 Listening Room":
        # Run the Listening Room interface
        run_listening_room()
    elif tab == "ℹ️ About":
